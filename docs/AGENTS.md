...
```
Seeder membuat akun `admin@sipp.sch.id` / `password`, tahun ajaran 2026/2027 (aktif) + semester Ganjil (`penilaian_dibuka`), dan 11 mapel plus kurikulum revisi client: Al-Qur'an, Eja Baghdadi, Fiqih, Tauhid, Akhlaq, Nahwu, Shorof, Tajwid, Tarikh, Bahasa Arab, Hadits (urutan rapor = urutan tsb; `JenisAssessmentSeeder` idempoten memberi tiap mapel UH/Tugas/UTS/UAS).
