        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <Select label="Kelas & Mapel" value={jadwalId ?? ""} onChange={(e) => { setJadwalId(Number(e.target.value) || null); setJenisId(null); }} placeholder="Pilih kelas & mapel">
              {jadwals.map((j) => (
                <option key={j.id} value={j.id}>{`${j.guru_mapel_kelas.mapel_plus.nama} · ${labelKelas(j.guru_mapel_kelas.kelas_rombel.nama) ?? j.guru_mapel_kelas.kelas_rombel.nama}`}</option>
              ))}
            </Select>
          </div>
          {jadwal && (
